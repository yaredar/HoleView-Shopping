...
const CommissionTaxPage: React.FC<CommissionTaxPageProps> = ({
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Rates are already pushed to store via onChange, but we can call a sync alert
    ...
    <div className="space-y-8 max-w-4xl text-left">
      <div className="bg-white p-10 rounded-[50px] border border-gray-100 shadow-sm space-y-8">
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1 text-left">Configure global financial deductions for all market streams</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-orange-50/50 p-8 rounded-[35px] border border-orange-100 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center bg-white px-3 py-1 rounded-xl border border-orange-200">
                <input 
                  type="number" 
                  step="0.1"
                  className="w-16 bg-transparent text-primary font-black text-xs outline-none text-right"
            ...
            />
            <p className="text-[9px] text-gray-400 font-medium text-left">Platform maintenance fee deducted from seller payouts.</p>
          </div>
          ...
          <div className="bg-green-50/50 p-8 rounded-[35px] border border-green-100 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center bg-white px-3 py-1 rounded-xl border border-green-200">
                <input 
                  type="number" 
                  step="0.1"
                  className="w-16 bg-transparent text-secondary font-black text-xs outline-none text-right"
                  value={otherFeeRate}
                  onChange={e => setOtherFeeRate(parseFloat(e.target.value) || 0)}
                />
            ...
            <input 
              ...
              value={otherFeeRate}
              onChange={e => setOtherFeeRate(parseFloat(e.target.value))}
            />
            <p className="text-[9px] text-gray-400 font-medium text-left">Standard processing and logistics fee applied at buyer checkout.</p>
          </div>
        </div>
        ...
        <button 
          onClick={handleSave}
          disabled={isSaving}
          className="w-full py-5 bg-gray-900 text-white rounded-[30px] font-black uppercase tracking-[0.2em] text-[12px] shadow-xl disabled:opacity-50"
        >
          {isSaving ? 'Synchronizing Nodes...' : 'Apply Global Configuration'}
        </button>
      </div>
    </div>
  );
