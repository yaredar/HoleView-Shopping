...
const Inbox: React.FC<InboxProps> = ({ chats, setChats, activeChatId, setActiveChatId, users }) => {
  ...
  useEffect(() => {
    try {
        const wsUrl = WS_URL(); // Explicitly call function to get string
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => handleSendMessage(undefined, { data: reader.result as string, name: file.name });
  ...
  const handleStartConversation = async () => {
    setLookupError('');
    const targetUser = users.find(u => u.phone.trim() === phoneLookup.trim());
    if (!targetUser) {
      setLookupError('User identifier (phone) not found in directory.');
      return;
    }
  ...
  return (
    <div className="card-premium flex h-[calc(100vh-200px)] md:h-[calc(100vh-250px)] overflow-hidden relative text-left">
      <div className={cn("w-full md:w-80 border-r border-slate-100 flex flex-col", activeChatId ? "hidden md:flex" : "flex")}>
        <div className="p-6 border-b border-slate-100 space-y-4">
          <button onClick={() => setShowNewChatModal(true)} className="btn-primary w-full text-[10px] flex items-center justify-center space-x-2">
            ...
            <span>Initialize User Comm</span>
          </button>
          <div className="relative">
            <input type="text" placeholder="Filter active streams..." className="input-standard py-2.5 pl-10 text-[10px] uppercase tracking-widest bg-slate-50" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {filteredChats.length === 0 ? (
            <div className="p-10 text-center"><p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Awaiting Uplink</p></div>
          ) : (
            filteredChats.map(chat => (
              <button key={chat.id} onClick={() => setActiveChatId(chat.id)} className={cn("w-full p-6 flex items-center space-x-4 hover:bg-slate-50 transition-colors border-b border-slate-50", activeChatId === chat.id && "bg-orange-50/50")}>
                <div className="w-12 h-12 rounded-2xl bg-white shadow-sm text-[#FF5722] border border-orange-50 flex-shrink-0 flex items-center justify-center font-black text-lg">{chat.contactName?.[0] || 'U'}</div>
      ...
      <div className={cn("flex-1 flex flex-col bg-slate-50/30", activeChatId ? "flex" : "hidden md:flex")}>
        {activeChat ? (
          <>
            <div className="p-5 bg-white border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div className="w-10 h-10 rounded-xl bg-[#FF5722] text-white flex items-center justify-center font-black">{activeChat.contactName?.[0] || 'U'}</div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
              {activeChat.messages.map(m => (
                <div key={m.id} className={cn("flex", m.isMe ? "justify-end" : "justify-start")}>
                  <div className={cn("max-w-[85%] md:max-w-[70%] p-4 rounded-[25px] shadow-sm text-xs font-medium", m.isMe ? "bg-[#FF5722] text-white rounded-tr-none" : "bg-white text-slate-700 rounded-tl-none border border-slate-100")}>
            ...
            <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100 flex items-center gap-2 md:gap-4">
              ...
              <textarea rows={1} placeholder="Secure uplink..." className="input-standard flex-1 p-3.5 bg-slate-50" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} />
              <button type="submit" disabled={!newMessage.trim()} className="bg-[#FF5722] text-white p-3.5 rounded-2xl shadow-xl active:scale-95 transition-transform"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg></button>
            </form>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-10 bg-white">
            ...
            <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest max-w-[200px] leading-relaxed">Select or initialize a user stream to begin communication</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-[40px] w-full max-sm p-8 shadow-2xl animate-scale-up text-left">
            <div className="flex justify-between items-center mb-8">
              <div>
                 ...
                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Privacy protocol: Enter user phone</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Target User (Phone)</label>
                 <input type="tel" placeholder="09..." className="input-standard text-sm uppercase tracking-widest p-4" value={phoneLookup} onChange={(e) => { setPhoneLookup(e.target.value); setLookupError(''); }} />
              </div>
              ...
              <button onClick={handleStartConversation} className="btn-primary w-full text-[10px]">Establish Bridge</button>
            </div>
          </div>
        </div>
      )}
