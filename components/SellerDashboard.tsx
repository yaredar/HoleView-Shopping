...
const SellerDashboard: React.FC<SellerDashboardProps> = ({ orders, setOrders, sellerName, products, sellerPhone }) => {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="bg-gray-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-premium">
         <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full -mr-32 -mt-32 blur-3xl"></div>
         
         <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
            <div>
               <h3 className="text-2xl font-black uppercase tracking-tighter">Performance Analysis</h3>
               <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">Provider Node: {sellerPhone}</p>
            </div>
            ...
            <div className="flex gap-10">
               ...
               <div className="text-center border-l border-white/10 pl-10">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-1">Node Health</p>
               </div>
            </div>
         </div>
      </div>

      {/* Rebalanced Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="bg-white p-6 rounded-3xl shadow-soft border border-gray-100 flex flex-col items-start">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">New Order Streams</p>
         </div>

         <div className="bg-white p-6 rounded-3xl shadow-soft border border-primary/20 flex flex-col items-start relative overflow-hidden group">
         ...
         <div className="bg-white p-6 rounded-3xl shadow-soft border border-gray-100 flex flex-col items-start">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">In Transit Streams</p>
         </div>
      </div>
      ...
      {paidLeads.length > 0 && (
         <div className="space-y-4">
            <div className="flex items-center gap-2 px-4">
               ...
               <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Priority Fulfillment Node</h4>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {paidLeads.map(o => (
                  <div key={o.id} className="bg-white p-6 rounded-2xl border border-primary/20 flex flex-col md:flex-row justify-between items-center gap-6 shadow-soft hover:border-primary transition-colors">
      ...
      <div className="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden">
         <div className="p-6 border-b border-gray-50 flex justify-between items-center">
            <h4 className="text-base font-black uppercase tracking-tighter text-gray-800">Inventory Stream History</h4>
         </div>
         <div className="overflow-x-auto no-scrollbar">
            <table className="w-full text-left whitespace-nowrap">
               <thead className="bg-gray-50 text-[10px] font-black uppercase tracking-widest text-gray-400">
                  <tr>
                     <th className="px-6 py-4">Node Stream</th>
                        ...
                        <tr key={o.id} className="hover:bg-gray-50/50 transition-colors">
                           ...
                           <td className="px-6 py-4 text-right">
                              {o.status === 'pending' && o.is_paid_confirmed && (
                                 <button onClick={() => handleShip(o.id)} className="text-[10px] font-black text-primary uppercase tracking-widest hover:underline">Dispatch Node</button>
                              )}
                              {o.status === 'pending' && !o.is_paid_confirmed && (
                                 <button onClick={() => handleReject(o.id)} className="text-[10px] font-black text-red-500 uppercase tracking-widest hover:underline">Abort Stream</button>
                              )}
                           </td>
                        </tr>
                     ))
