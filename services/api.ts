 ...
 * Primary endpoint for Cluster Communication.
 */
export const getApiOrigin = () => {
  const saved = localStorage.getItem(STORAGE_KEY);
  if (saved) return saved;
  
  // Default to domain to avoid IP preflight redirects
...
async function request(endpoint: string, options: RequestInit = {}) {
  ...
  const url = `${origin}/api${cleanEndpoint}`;
...
export const api = {
  ...
  async login(phone: string, key: string) { 
  ...
  async updateProfile(u: Partial<User>) { 
    return request('/users/profile', { method: 'POST', body: JSON.stringify(u) }); 
  },

  async approveVerification(user_id: string, status: VerificationStatus) { 
    return request('/users/verify', { method: 'POST', body: JSON.stringify({ user_id, verification_status: status }) }); 
  },

  async changePassword(user_id: string, password: string) { 
    return request('/users/password', { method: 'POST', body: JSON.stringify({ user_id, password }) }); 
  },
  ...
  async checkout(orders: Order[]) { 
    return request('/checkout', { method: 'POST', body: JSON.stringify(orders) }); 
  },
  ...
  async addAd(a: Ad) { 
    return request('/ads', { method: 'POST', body: JSON.stringify(a) }); 
  },

  async toggleAd(id: string, isActive: boolean) { 
    return request('/ads/toggle', { method: 'POST', body: JSON.stringify({ id, isActive }) }); 
  },

  async deleteAd(id: string) { 
    return request('/ads/delete', { method: 'POST', body: JSON.stringify({ id }) }); 
  },
  ...
  async addSubscription(s: Subscription) { 
    return request('/subscriptions', { method: 'POST', body: JSON.stringify(s) }); 
  },
  ...
  async saveChat(c: ChatThread) { 
    return request('/chats', { method: 'POST', body: JSON.stringify(c) }); 
  },
