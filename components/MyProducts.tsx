...
const MyProducts: React.FC<MyProductsProps> = ({ products, setProducts, sellerPhone, searchTerm }) => {
  ...
  const myProductsFiltered = useMemo(() => {
    // Ensure we match strictly by phone number
    return products.filter(p => {
      const isMine = String(p.seller_phone).trim() === String(sellerPhone).trim();
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Terminate this listing feed?")) {
    ...
    <div className="space-y-6 text-left animate-scale-up pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {myProductsFiltered.length === 0 ? (
            <div className="col-span-full py-24 text-center bg-white rounded-[40px] border-2 border-dashed">
                <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Inventory Stream Offline</p>
            </div>
        ) : (
            myProductsFiltered.map(product => (
              <div key={product.id} className="bg-white rounded-[40px] overflow-hidden shadow-soft border border-slate-50 flex flex-col group">
                 ...
                 <div className="p-6 flex flex-col flex-1">
                    <h3 className="font-black text-slate-800 uppercase tracking-tighter mb-4 line-clamp-1">{product.name}</h3>
