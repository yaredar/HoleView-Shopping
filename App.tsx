...
const App: React.FC = () => {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (password.length < 4) { alert("New Key must be at least 4 chars."); return; }
    
    setIsProcessing(true);
    try {
      const res = await api.resetPassword(phone, firstName, password);
      if (res.success) {
        alert("Success: Access Key has been updated. Please sign in.");
        setAuthMode('signin');
      }
    } catch (err: any) {
      alert("Verification Failed: Could not find user with that phone and first name.");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  if (!store.isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900 overflow-y-auto py-12">
        <div className="card-premium w-full max-w-md p-8 shadow-2xl animate-scale-up border-slate-800 relative">
          <div className="flex flex-col items-center mb-8">
            ...
            <div className="flex items-center gap-2 mt-4 text-[9px] font-black uppercase tracking-widest text-slate-400">
          ...
          {authMode === 'signin' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="submit" disabled={isProcessing} className="btn-primary w-full text-xs">Login</button>
              <div className="text-center mt-4">
                <button type="button" onClick={() => setAuthMode('forgot')} className="text-[10px] font-black text-slate-400 hover:text-[#FF5722] uppercase tracking-widest">Forgot Password?</button>
              </div>
            </form>
          )}
          ...
          {authMode === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <input placeholder="First Name" className="input-standard" value={firstName} onChange={e => setFirstName(e.target.value)} required />
                <input placeholder="Last Name" className="input-standard" value={lastName} onChange={e => setLastName(e.target.value)} required />
              </div>
              <input type="tel" placeholder="Phone Number" className="input-standard" value={phone} onChange={e => setPhone(e.target.value)} required />
              <select className="input-standard text-xs uppercase" value={selectedRole} onChange={e => setSelectedRole(e.target.value as UserRole)}>
                <option value={UserRole.BUYER}>Buyer</option>
                <option value={UserRole.SELLER}>Seller</option>
              </select>
              <input type="password" placeholder="Key" className="input-standard" value={password} onChange={e => setPassword(e.target.value)} required />
              <input type="password" placeholder="Confirm" className="input-standard" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
              <button type="submit" disabled={isProcessing} className="btn-primary !bg-emerald-500 w-full text-xs">Register</button>
            </form>
          )}
          ...
          {authMode === 'forgot' && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="text-center mb-6">
                <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest">Identify Recovery</h2>
              </div>
              <input type="tel" placeholder="Registered Phone" className="input-standard" value={phone} onChange={e => setPhone(e.target.value)} required />
              <input placeholder="Registered First Name" className="input-standard" value={firstName} onChange={e => setFirstName(e.target.value)} required />
              <div className="pt-4 border-t border-slate-700">
                <input type="password" placeholder="New Access Key" className="input-standard mb-3" value={password} onChange={e => setPassword(e.target.value)} required />
                <input type="password" placeholder="Confirm New Key" className="input-standard" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
              </div>
              <button type="submit" disabled={isProcessing} className="btn-primary w-full text-xs">Reset Access Key</button>
              <button type="button" onClick={() => setAuthMode('signin')} className="w-full py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 hover:text-white transition-colors">Back to Login</button>
            </form>
          )}
        </div>
      </div>
      ...
      {store.isSyncing && (
        <div className="fixed bottom-24 right-8 z-[200]">
          <div className="bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700 flex items-center gap-2 shadow-2xl">
             ...
             <span className="text-[8px] font-black text-white uppercase tracking-widest">Refreshing</span>
          </div>
        </div>
      )}
      {content}
